...
class Agent(Node):
    ...
    def map_update(self):
        """ Update the agent's local map using LiDAR data """
        if self.x is None or self.y is None or self.yaw is None:
            self.get_logger().warn("No valid position data yet!")
            return
        ...
        for i, distance in enumerate(self.lidar_data.ranges):
            ...
            i_grid = int((x_global - self.map_msg.info.origin.position.x) / self.map_msg.info.resolution)
            j_grid = int((y_global - self.map_msg.info.origin.position.y) / self.map_msg.info.resolution)
            if 0 <= i_grid < self.w and 0 <= j_grid < self.h:
                self.map[j_grid, i_grid] = 100  # Mark cell as occupied
            else:
                self.get_logger().warn(f"Obstacle out of bounds: ({i_grid}, {j_grid})")
